...
func main() {
	...
	if err != nil {
		logger.Errorf("netstack.CreateNetTUN - %v", err)
		os.Exit(1)
	}

	dev := device.NewDevice(tun, conn.NewDefaultBind(), device.NewLogger(logLevel, fmt.Sprintf("(%s) ", *interfaceName)))
...
`)
	...
	if err != nil {
		logger.Errorf("netstack.Net.ListenTCP - %v", err)
		os.Exit(1)
	}
	...
	go func() {
		http.HandleFunc("/", func(writer http.ResponseWriter, request *http.Request) {
			...
			_, _ = io.WriteString(writer, "Hello from userspace netstack TCP!")
		})
